...
import { actionService, makeActionManager } from "@web/webclient/actions/action_service"

console.log('makeActionManager', makeActionManager)
...
function customMakeActionManager(env) {
    ...
    async function _triggerDownload(action, options, type) {
        ...
        try {
            return doAction({ type: "ir.actions.act_url", url: url, target: 'new' });
            await download({
                url: "/report/download",
                data: {
...
/* TODO:Arash; It changes the _triggerDownload function to redirect to report tab instead of download the pdf file.
    It is needed to add export before the makeActionManager function.
*/
patch(actionService, 'arash', {
    dependencies: ["effect", "localization", "notification", "router", "rpc", "ui", "user"],
    start(env) {
        return customMakeActionManager(env);
    },
})
